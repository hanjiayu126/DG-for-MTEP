function [elem,bdFlag,elem2edge,Dlambda,elem2face] = sortelem3(elem,bdFlag,elem2edge,Dlambda,elem2face)
...
%%added by Han
if nargin==5
[~,l] = sort(elem,2); locEdge = [1 2; 1 3; 1 4; 2 3; 2 4; 3 4]; NT=size(elem,1);
s=sparse(locEdge(:,1),locEdge(:,2),1:6,4,4); s=s+sparse(locEdge(:,2),locEdge(:,1),1:6,4,4);
elem2edge=elem2edge([(1:NT)' (1:NT)' (1:NT)' (1:NT)' (1:NT)' (1:NT)']+[s(l(:,1)+l(:,2)*4-4)-1 s(l(:,1)+l(:,3)*4-4)-1....
    s(l(:,1)+l(:,4)*4-4)-1 s(l(:,2)+l(:,3)*4-4)-1  s(l(:,2)+l(:,4)*4-4)-1 s(l(:,3)+l(:,4)*4-4)-1]*NT);
% Dlambda1(:,:,1)=reshape(Dlambda(:,1,:),size(elem,1),4);
% Dlambda1(:,:,2)=Dlambda(:,2,:);clear Dlambda
Dlambdax=Dlambda(:,1,:); Dlambday=Dlambda(:,2,:);
Dlambda(:,1,:)=Dlambdax([(1:NT)' (1:NT)' (1:NT)' (1:NT)'  (1:NT)'  (1:NT)']+[s(l(:,1)+l(:,2)*4-4)-1 s(l(:,1)+l(:,3)*4-4)-1....
    s(l(:,1)+l(:,4)*4-4)-1 s(l(:,2)+l(:,3)*4-4)-1  s(l(:,2)+l(:,4)*4-4)-1 s(l(:,3)+l(:,4)*4-4)-1]*NT);
Dlambda(:,2,:)=Dlambday([(1:NT)' (1:NT)' (1:NT)' (1:NT)'  (1:NT)'  (1:NT)']+[s(l(:,1)+l(:,2)*4-4)-1 s(l(:,1)+l(:,3)*4-4)-1....
    s(l(:,1)+l(:,4)*4-4)-1 s(l(:,2)+l(:,3)*4-4)-1  s(l(:,2)+l(:,4)*4-4)-1 s(l(:,3)+l(:,4)*4-4)-1]*NT);
